...
[Pinpoint](docs/PINPOINT_UI.md) lets you mark numbered points and areas on any plot in the study and refer to them in a request, such as "label 1" or "why is 2 so high?". The assistant sees the marks on the plot image and, for plots drawn in R, their data coordinates. For embeddings and spatial maps of millions of cells, Pinpoint draws point maps on the GPU over a tissue image; clicking a cell or dragging an area gives the assistant the exact rows.
